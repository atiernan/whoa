...
@vertex fn main(@location(0) vs_v0_: vec4<f32>, @location(2) @interpolate(flat) vs_v1_u: vec4<u32>, @location(3) vs_v2_: vec4<f32>, @location(6) vs_v3_: vec4<f32>) -> VertexOutput {
    ...
    vs_r3_.y = _e177.y;
    vs_r3_.z = _e177.z;
    let _e200 = clamp(((vs_r1_.xxx * vs_r3_.xyz) + global.vs_uniforms_vec4_[11].xyz), vec3(0f), vec3(1f));
    vs_r1_.x = _e200.x;
    vs_r1_.y = _e200.y;
    vs_r1_.z = _e200.z;
    vs_r1_.w = 1f;
    ...
    vs_r1_.z = local;
    let _e392 = (vs_r1_.xy * vs_r1_.zz);
    vs_r1_.x = _e392.x;
    vs_r1_.y = _e392.y;
    let _e414 = ((vs_r1_.xy * vec2<f32>(0.5f, 0.5f)) + vec2<f32>(0.5f, 0.5f));
    vs_o3_.x = _e414.x;
    vs_o3_.y = _e414.y;
