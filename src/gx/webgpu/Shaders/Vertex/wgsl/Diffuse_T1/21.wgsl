...
@vertex fn main(@location(0) vs_v0_: vec4<f32>, @location(1) vs_v1_: vec4<f32>, @location(2) @interpolate(flat) vs_v2_u: vec4<u32>, @location(3) vs_v3_: vec4<f32>, @location(6) vs_v4_: vec4<f32>) -> VertexOutput {
    ...
    let _e220 = normalize(vs_r0_.xyz);
    vs_r2_.x = _e220.x;
    vs_r2_.y = _e220.y;
    vs_r2_.z = _e220.z;
    vs_r0_.x = clamp(dot(-(global.vs_uniforms_vec4_[12].xyz), vs_r2_.xyz), 0f, 1f);
    let _e244 = global.vs_uniforms_vec4_[10].xyz;
    vs_r2_.x = _e244.x;
    vs_r2_.y = _e244.y;
    vs_r2_.z = _e244.z;
    let _e267 = clamp(((vs_r0_.xxx * vs_r2_.xyz) + global.vs_uniforms_vec4_[11].xyz), vec3(0f), vec3(1f));
    vs_r0_.x = _e267.x;
    vs_r0_.y = _e267.y;
    vs_r0_.z = _e267.z;
    vs_r1_.w = 1f;
