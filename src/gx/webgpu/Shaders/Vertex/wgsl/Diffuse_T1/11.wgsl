...
@vertex fn main(@location(0) vs_v0_: vec4<f32>, @location(2) @interpolate(flat) vs_v1_u: vec4<u32>, @location(3) vs_v2_: vec4<f32>, @location(6) vs_v3_: vec4<f32>) -> VertexOutput {
    ...
    let _e184 = normalize(vs_r0_.xyz);
    vs_r1_.x = _e184.x;
    vs_r1_.y = _e184.y;
    vs_r1_.z = _e184.z;
    vs_r0_.x = clamp(dot(-(global.vs_uniforms_vec4_[12].xyz), vs_r1_.xyz), 0f, 1f);
    let _e208 = global.vs_uniforms_vec4_[10].xyz;
    vs_r1_.x = _e208.x;
    vs_r1_.y = _e208.y;
    vs_r1_.z = _e208.z;
    let _e231 = clamp(((vs_r0_.xxx * vs_r1_.xyz) + global.vs_uniforms_vec4_[11].xyz), vec3(0f), vec3(1f));
    vs_r0_.x = _e231.x;
    vs_r0_.y = _e231.y;
    vs_r0_.z = _e231.z;
    vs_r0_.w = 1f;
