...
@vertex fn main(@location(0) vs_v0_: vec4<f32>, @location(1) vs_v1_: vec4<f32>, @location(2) @interpolate(flat) vs_v2_u: vec4<u32>, @location(3) vs_v3_: vec4<f32>, @location(6) vs_v4_: vec4<f32>, @location(7) vs_v5_: vec4<f32>) -> VertexOutput {
    ...
    let _e246 = global.vs_uniforms_vec4_[10].xyz;
    vs_r2_.x = _e246.x;
    vs_r2_.y = _e246.y;
    vs_r2_.z = _e246.z;
    let _e269 = clamp(((vs_r0_.xxx * vs_r2_.xyz) + global.vs_uniforms_vec4_[11].xyz), vec3(0f), vec3(1f));
    vs_r0_.x = _e269.x;
    vs_r0_.y = _e269.y;
    vs_r0_.z = _e269.z;
    vs_r1_.w = 1f;
