...
@vertex fn main(@location(0) vs_v0_: vec4<f32>, @location(3) vs_v1_: vec4<f32>, @location(6) vs_v2_: vec4<f32>, @location(7) vs_v3_: vec4<f32>) -> VertexOutput {
    ...
    let _e103 = normalize(vs_r2_.xyz);
    vs_r3_.x = _e103.x;
    vs_r3_.y = _e103.y;
    vs_r3_.z = _e103.z;
    vs_r2_.x = clamp(dot(-(global.vs_uniforms_vec4_[12].xyz), vs_r3_.xyz), 0f, 1f);
    let _e127 = global.vs_uniforms_vec4_[10].xyz;
    vs_r4_.x = _e127.x;
    vs_r4_.y = _e127.y;
    vs_r4_.z = _e127.z;
    let _e145 = ((vs_r2_.xxx * vs_r4_.xyz) + global.vs_uniforms_vec4_[11].xyz);
    vs_r2_.x = _e145.x;
    vs_r2_.y = _e145.y;
    vs_r2_.z = _e145.z;
    let _e161 = (-(vs_r0_.xyz) + global.vs_uniforms_vec4_[21].xyz);
    vs_r4_.x = _e161.x;
    vs_r4_.y = _e161.y;
    vs_r4_.z = _e161.z;
    vs_r2_.w = dot(vs_r4_.xyz, vs_r3_.xyz);
    let _e177 = vs_r3_.xyz;
    vs_o5_.x = _e177.x;
    vs_o5_.y = _e177.y;
    vs_o5_.z = _e177.z;
    vs_r3_.x = dot(vs_r4_.xyz, vs_r4_.xyz);
    if (vs_r3_.x == 0f) {
        local = FLT_MAX;
    } else {
    ...
    vs_r3_.y = local;
    vs_r2_.w = (vs_r2_.w * vs_r3_.y);
    vs_r3_.y = (vs_r3_.x * vs_r3_.y);
    vs_r3_.y = (vs_r3_.y * global.vs_uniforms_vec4_[26].x);
    vs_r3_.x = ((vs_r3_.x * global.vs_uniforms_vec4_[27].x) + vs_r3_.y);
    vs_r3_.x = (vs_r3_.x + global.vs_uniforms_vec4_[25].x);
    if (vs_r3_.x == 0f) {
        local_1 = FLT_MAX;
    } else {
        local_1 = (1f / vs_r3_.x);
    ...
    vs_r2_.w = (vs_r3_.x * vs_r2_.w);
    let _e282 = clamp(((global.vs_uniforms_vec4_[17].xyz * vs_r2_.www) + vs_r2_.xyz), vec3(0f), vec3(1f));
    vs_r1_.x = _e282.x;
    vs_r1_.y = _e282.y;
    vs_r1_.z = _e282.z;
    ...
    let _e361 = ((vs_v2_.xyx * vec3<f32>(1f, 1f, 0f)) + vec3<f32>(0f, 0f, 1f));
    vs_r1_.x = _e361.x;
    vs_r1_.y = _e361.y;
    vs_r1_.z = _e361.z;
    vs_o2_.x = dot(global.vs_uniforms_vec4_[6].xyw, vs_r1_.xyz);
    vs_o2_.y = dot(global.vs_uniforms_vec4_[7].xyw, vs_r1_.xyz);
    let _e401 = ((vs_v3_.xyx * vec3<f32>(1f, 1f, 0f)) + vec3<f32>(0f, 0f, 1f));
    vs_r1_.x = _e401.x;
    vs_r1_.y = _e401.y;
    vs_r1_.z = _e401.z;
