...
@vertex fn main(@location(0) vs_v0_: vec4<f32>, @location(1) vs_v1_: vec4<f32>, @location(2) @interpolate(flat) vs_v2_u: vec4<u32>, @location(3) vs_v3_: vec4<f32>, @location(6) vs_v4_: vec4<f32>, @location(7) vs_v5_: vec4<f32>) -> VertexOutput {
    ...
    let _e445 = clamp(((global.vs_uniforms_vec4_[17].xyz * vs_r2_.xxx) + vs_r3_.xyz), vec3(0f), vec3(1f));
    vs_r0_.x = _e445.x;
    vs_r0_.y = _e445.y;
    vs_r0_.z = _e445.z;
    ...
    let _e524 = ((vs_v4_.xyx * vec3<f32>(1f, 1f, 0f)) + vec3<f32>(0f, 0f, 1f));
    vs_r0_.x = _e524.x;
    vs_r0_.y = _e524.y;
    vs_r0_.z = _e524.z;
    vs_o2_.x = dot(global.vs_uniforms_vec4_[6].xyw, vs_r0_.xyz);
    vs_o2_.y = dot(global.vs_uniforms_vec4_[7].xyw, vs_r0_.xyz);
    let _e564 = ((vs_v5_.xyx * vec3<f32>(1f, 1f, 0f)) + vec3<f32>(0f, 0f, 1f));
    vs_r0_.x = _e564.x;
    vs_r0_.y = _e564.y;
    vs_r0_.z = _e564.z;
