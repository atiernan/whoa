...
void CGxDeviceWebGPU::IEnsureDxtPipeline() {
    ...
    if (!SFile::Load(nullptr, "/Shaders/Compute/DxtDecompress.wgsl", &shaderData, &shaderSize, 0, 0x1, nullptr) || !shaderData) {
        CGxDevice::Log("WebGPU: Failed to load DXT shader file");
        if (shaderData) {
            SFile::Unload(shaderData);
        }
        return;
    }
    ...
    wgslSource.code = { static_cast<const char*>(shaderData), shaderSize };
...
void CGxDeviceWebGPU::ShaderCreate(CGxShader* shaders[], EGxShTarget target, const char* basePath, const char* shaderName, int32_t permutations) {
    ...
    for (int32_t p = 0; p < permutations; p++) {
        ...
        if (SFile::Load(nullptr, path, &shaderData, &shaderSize, 0, 0x1, nullptr) && shaderData) {
            shader->code.SetCount(static_cast<uint32_t>(shaderSize));
            memcpy(shader->code.Ptr(), shaderData, shaderSize);
            SFile::Unload(shaderData);
